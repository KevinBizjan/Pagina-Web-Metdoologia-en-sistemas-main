...
exports.create = (req, res) => {
    ...
    if (!alumno_nombre || !alumno_edad || !nivel || !turno || !tutor_nombre || !tutor_telefono || !tutor_email) {
        return res.status(400).json({ message: 'Todos los campos obligatorios deben ser completados' });
    }

    const query = `INSERT INTO preinscripciones 
        (alumno_nombre, alumno_edad, nivel, turno, tutor_nombre, tutor_telefono, tutor_email, observaciones) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;
    
    db.run(query, [alumno_nombre, alumno_edad, nivel, turno, tutor_nombre, tutor_telefono, tutor_email, observaciones], function(err) {
        if (err) {
